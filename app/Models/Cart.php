...
// **************** ADD TO CART *******************
    ...
    public function add($item, $id, $size,$size_qty,$color, $var,$keys, $values) {
        $co_qty = Color::where('product_id',$id)->where('size','=',$size)->where('colors','#'.$color)->first();
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color,'var' => $var, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item->toArray(), 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
        ...
        $size_cost = $item->size_price[0];
            ...
            if(!empty($co_qty)){
        $storedItem['size_price'] = $co_qty->size_price;    
        $size_cost = $co_qty->size_price;
        }
        if(!empty($color)){
...
// **************** ADD TO CART MULTIPLE *******************
    ...
    public function addnum($item, $id, $qty, $size, $color,$var, $size_qty, $size_price, $size_key, $keys, $values, $image= null) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color,'var' => $var, 'stock' => $item->stock, 'price' => $item->price, 'item' => $item->toArray(), 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values, 'image' => $image];
        if($item->type == 'Physical')
        {
            if ($this->items) {
                if (array_key_exists($id.$size.$color.str_replace(str_split(' ,'),'',$values), $this->items)) {
