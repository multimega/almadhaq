...
class MessageController extends Controller
{
   ...
   public function messages_34()
    {
        $user = Auth::guard('web')->user();
        $convs = Conversation::where('sent_user','=',$user->id)->orWhere('recieved_user','=',$user->id)->get();
        return view('user.message.index_34',compact('user','convs'));            
    }
    ...
    public function message_34($id)
    {
            $user = Auth::guard('web')->user();
            $conv = Conversation::findOrfail($id);
            return view('user.message.create_34',compact('user','conv'));                 
    }
    ...
    public function adminmessages_34()
    {
            $user = Auth::guard('web')->user();
            $convs = AdminUserConversation::where('type','=','Ticket')->where('user_id','=',$user->id)->get();
            return view('user.ticket.index_34',compact('convs'));            
    }
    ...
    public function adminDiscordmessages_34()
    {
            $user = Auth::guard('web')->user();
            $convs = AdminUserConversation::where('type','=','Dispute')->where('user_id','=',$user->id)->get();
            return view('user.dispute.index_34',compact('convs'));            
    }
    ...
    public function adminmessage_34($id)
    {
            $conv = AdminUserConversation::findOrfail($id);
            return view('user.ticket.create_34',compact('conv'));                 
    }   
