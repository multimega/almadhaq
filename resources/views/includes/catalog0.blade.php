<div class="col-lg-3 col-md-6">
  <div class="left-area">
          ...
          <ul class="filter-list">
            ...
            <li>
              <div class="content">
                @if(!$slang)
                @if($lang->id == 2)
                <a href="{{route('front.category', ['category' => $element->slug_ar , 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                  class="category-link"> <i class="fas fa-angle-double-right"></i>
                  {{$element->name_ar}}
                  @else
                  <a href="{{route('front.category', ['category' => $element->slug , 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                    class="category-link"> <i class="fas fa-angle-double-right"></i>
                    {{$element->name}}
                    @endif
                    @else
                    ...
                    <a href="{{route('front.category',  ['category' => $element->slug_ar , 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                      ...
                      <div class="sub-content open">
                        @if(!$slang)
                        @if($lang->id == 2)
                        <a href="{{route('front.category', ['category' => $cat->slug_ar ,'subcategory' =>  $subelement->slug_ar, 'lang' => $sign ] )}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                          class="subcategory-link"><i class="fas fa-angle-right"></i>
                          {{$subelement->name_ar}}
                          @else
                          <a href="{{route('front.category', ['category' => $cat->slug ,'subcategory' =>  $subelement->slug, 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                            class="subcategory-link"><i class="fas fa-angle-right"></i>
                            {{$subelement->name}}
                            @endif
                            @else
                            ...
                            <a href="{{route('front.category', ['category' => $cat->slug_ar ,'subcategory' =>  $subelement->slug_ar, 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                              ...
                              <div class="child-content open">
                                @if(!$slang)
                                @if($lang->id == 2)
                                <a href="{{route('front.category',  ['category' => $cat->slug_ar ,'subcategory' => $subcat->slug_ar ,'childcategory' =>  $childcat->slug_ar, 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                                  class="subcategory-link"><i class="fas fa-caret-right"></i>
                                  {{$childcat->name_ar}}
                                  @else
                                  <a href="{{route('front.category', ['category' => $cat->slug ,'subcategory' => $subcat->slug ,'childcategory' =>  $childcat->slug, 'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}"
                                    class="subcategory-link"><i class="fas fa-caret-right"></i>
                                    {{$childcat->name}}
                                    @endif
                                    @else
    ...
    !empty(json_decode($subcat->attributes, true))) || (!empty($childcat) && !empty(json_decode($childcat->attributes,
    true))))
          ...
          <ul class="filter">
            <div class="single-filter">
              ...
              <div class="form-check ml-0 pl-0">
                ...
                <label class="form-check-label" for="{{$attr->input_name}}{{$option->id}}">@if(!$slang)
                  @if($lang->id == 2)
                  {{ $option->name_ar }}
                  @else
                  {{ $option->name }}
    ...
    {{-- <div class="tags-area">
      ...
      <div class="body-area">
        <ul class="taglist">
          ...
          @if(!empty($tag))
          <li>
            <a class="{{ isset($tags) ? ($tag == $tags ? 'active' : '') : ''}}"
              href="{{ route('front.tag',['slug' => $tag , 'lang' => $sign ]) }}">
              {{ $tag }}
            </a>
          </li>
          @endif
