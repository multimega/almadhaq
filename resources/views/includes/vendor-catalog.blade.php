          ...
          <div class="left-area">
            ...
            <div class="body-area">
                ...
                <ul class="filter-list">
                  ...
                  <li>
                    <div class="content">
                        <a href="{{route('front.category', ['category' => $element->slug ,'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-angle-double-right"></i> {{$element->name}}</a>
                        @if(!empty($cat) && $cat->id == $element->id && !empty($cat->subs))
                            @foreach ($cat->subs as $key => $subelement)
                            <div class="sub-content open">
                              <a href="{{route('front.category', ['category' => $element->slug ,'subcategory' => $subelement->slug ,'lang' => $sign ])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-angle-right"></i>{{$subelement->name}}</a>
                              @if(!empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs))
                                ...
                                <div class="child-content open">
                                  <a href="{{route('front.category', ['category' => $element->slug ,'subcategory' => $subelement->slug,'childcategory' => $childelement->slug ,'lang' => $sign ] )}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-caret-right"></i>{{$childcat->name}}</a>
                                </div>
                                @endforeach
                              @endif
                            </div>
              ...
              <form class="price-range-block" id="priceForm" action="{{ route('front.vendor',['slug' =>  Request::route('category')]) }}">
