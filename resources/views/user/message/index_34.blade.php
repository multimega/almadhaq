...
@section('content')
                ...
                <div>
                    <div class="settings-container">
                        <div class="settings-header d-flex justify-content-between align-items-center">
                            <h3>{{ $langg->lang356 }}</h3>
                            <a data-toggle="modal" data-target="#vendorform" class="btn btn-success" href="javascript:;"> <i
                                    class="fas fa-envelope"></i> {{ $langg->lang357 }}</a>
                        </div>
                                        ...
                                        <tbody>
                                            @foreach ($convs as $conv)
                                                <tr class="conv">
                                                    ...
                                                    <td>
                                                        <a href="{{ route('user-message-34', $conv->id) }}"
                                                            class="link view"><i class="fas fa-eye text-success"></i></a>
                                                        <a href="javascript:;" data-toggle="modal"
                                                            data-target="#confirm-delete"
                                                            data-href="{{ route('user-message-delete', $conv->id) }}"
                                                            class="link remove"><i class="fas fa-trash text-danger"></i></a>
                                                    </td>
                                                </tr>
                                            @endforeach
                                        </tbody>
    ...
    <div class="message-modal">
        <div class="modal fade" id="vendorform" tabindex="-1" role="dialog" aria-labelledby="vendorformLabel"
            aria-hidden="true">
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="contact-form">
                                        <form id="emailreply">
                                            {{ csrf_field() }}
                                            <input type="text" class="input-field form-control mb-3" id="eml"
                                                name="email" placeholder="{{ $langg->lang363 }} *"
                                                value="{{ Auth::guard('web')->user()->phone }}" required="">
                                            <input type="text" class="input-field form-control mb-3" id="subj"
                                                name="subject" placeholder="{{ $langg->lang364 }} *" required="">
                                            <textarea class="input-field textarea form-control mb-3" name="message" id="msg"
                                                placeholder="{{ $langg->lang365 }} *" required=""></textarea>
                                            <input type="hidden" name="name"
                                                value="{{ Auth::guard('web')->user()->name }}">
                                            <input type="hidden" name="user_id"
                                                value="{{ Auth::guard('web')->user()->id }}">
                                            <button class="submit-btn btn btn-success" id="emlsub"
