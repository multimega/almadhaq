...
@section('content')  
          <div class="content-area">
                <div class="home-head mb-4 mb-md-5">
                    <h3>{{ __('Customers Cart') }} <span>{{ __('Manage your customer cart') }}</span></h3>
                    <nav class="mt-3" style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
                        <ol class="breadcrumb">
                            <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __("Dashboard") }}</a></li>
                            <li class="breadcrumb-item">{{ __('Customers Cart') }}</li>
                        </ol>
                    </nav>
                </div>
  ...
  <div class="modal-header d-block text-center">
    <h4 class="modal-title d-inline-block">{{ __('Confirm Delete') }}</h4>
      ...
      <div class="modal-body">
            <p class="text-center">{{ __('You are about to delete this Cart Data.') }}</p>
...
@section('scripts')
    ...
    var table = $('#geniustable').DataTable({
         ordering: false,
               processing: true,
               serverSide: true,
               ajax: '{{ route('admin-user-cart-datatables') }}',
               columns: [
                  { data: 'name', name: 'name' },
