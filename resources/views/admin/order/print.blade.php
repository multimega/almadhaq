                    ...
                    <div class="table-responsive">
                        <table id="example2" class="table table-hover dt-responsive" cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td colspan="1"></td>
                                    <td><strong>{{ __('Total without VAT') }}</strong></td>
                                    <td>{{ number_format(abs((round($subtotal, 2) - (round($order->pay_amount, 2) * $order->currency_value - $order->shipping_price) - round($order->pay_amount, 2) * $order->currency_value - $order->shipping_price) / 1.15), 2) }}
                                    </td>
                                </tr>
                                </tr>
                                ...
                                @if ($order->shipping_cost != 0)
                                    <tr class="no-border">
                                        <td colspan="1"></td>
                                        <td><strong>{{ __('Shipping Cost') }}({{ $order->currency_sign }})</strong>
                                        </td>
                                ...
                                @if ($order->packing_cost != 0)
                                    <tr class="no-border">
                                        <td colspan="1"></td>
                                        <td><strong>{{ __('Packaging Cost') }}({{ $order->currency_sign }})</strong>
                                        </td>
                                ...
                                @if ($order->wallet != 0)
                                    <tr class="no-border">
                                        <td colspan="1"></td>
                                        <td><strong>{{ __('Wallet Discount') }}({{ $order->currency_sign }})</strong>
                                        </td>
                                ...
                                @if ($order->shipping_price != 0)
                                    <tr class="no-border">
                                        <td colspan="1"></td>
                                        <td><strong>{{ __('Shipping Price') }}({{ $order->currency_sign }})</strong>
                                        </td>
                                ...
                                <tr>
                                    ...
                                    <td><strong>{{ __('VAT value') }}</strong></td>
                                    <td>{{ $order->currency_sign }}
                                        {{ round(round($order->pay_amount, 2) * $order->currency_value - $order->shipping_price - (round($order->pay_amount, 2) * $order->currency_value) / 1.15, 2) }}
                                </tr>
