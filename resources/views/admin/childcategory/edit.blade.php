...
<form id="geniusformdata" action="{{route('admin-childcat-update',$data->id)}}" method="POST" enctype="multipart/form-data" class="inp-50">
    {{csrf_field()}}
    ...
    <div class="row">
        <div class="col-lg-12">
            ...
            <select id="subcat" class="form-select" name="subcategory_id" required="">
                <option value="">{{ __('Select Sub Category') }}</option>
                @foreach($data->subcategory->category->subs as $sub)
                  <option value="{{$sub->id}}" {{$sub->id == $data->subcategory->id ? "selected":""}}>{{$sub->name}}</option>
                @endforeach
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <div class="img-upload">
                <div id="image-preview" class="img-preview" style="background: url({{ $data->photo ? asset('assets/images/childcategories/'.$data->photo):asset('assets/images/noimage.png') }});">
