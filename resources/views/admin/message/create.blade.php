...
<div class="content-area">
    <div class="home-head mb-4 mb-md-5">
        ...
        <h3>{{ __('Order Number') }}: {{$conv->order_number}}</span></h3>
        @endif
        <h3>{{ __('Conversation with') }} {{isset($conv->user) ? $conv->user->name : 'hidden'}} <span>Manage your conversation</span></h3>
        <nav class="mt-3" style="--bs-breadcrumb-divider: '>';" aria-label="breadcrumb">
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="{{ route('admin.dashboard') }}">{{ __("Dashboard") }}</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin.support-manager') }}">{{ __('Support Manager') }}</a></li>
                <li class="breadcrumb-item"><a href="{{ route('admin-message-index') }}">{{ __("Messages") }}</a></li>
                <li class="breadcrumb-item">{{ __("Conversations Details") }}</li>
            </ol>
        </nav>
    </div>

    <div class="support-ticket-wrapper default-box">
        <div class="panel panel-primary">
            ...
            <div class="panel-footer">
                <form id="messageform" action="{{route('admin-message-store')}}" data-href="{{ route('admin-message-load',$conv->id) }}" class="exp-form" method="POST">
                    {{csrf_field()}}
                    ...
                    <div class="row">
						<div class="col-lg-12 text-center">
							<button class="main-light-btn py-3" type="submit">{{ __('Add Reply') }} <i class="fas fa-check ms-3"></i></button>
						</div>
					</div>
                </form>
            </div>
