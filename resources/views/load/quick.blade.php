    ...
    <div class="xzoom-container">
        ...
        <div class="carousel-inner">
            <!--<div class="carousel-item active">
                <img id="xzoom-magnific1" src="{{filter_var($product->photo, FILTER_VALIDATE_URL) ?$product->photo:asset('assets/images/products/'.$product->photo)}}"/>
            </div>
            @foreach($product->galleries as $gal)
                <div class="carousel-item">
                    <img id="xzoom-magnific{{ $gal->id }}" src="{{filter_var($gal->photo, FILTER_VALIDATE_URL) ?$gal->photo:asset('assets/images/products/'.$gal->photo)}}"/>
                </div>
            @endforeach-->

            <div class="carousel-item active">
                <img class=" d-block w-100" src="{{filter_var($product->photo, FILTER_VALIDATE_URL) ?$product->photo:asset('assets/images/products/'.$product->photo)}}"  @if(!$slang) @if($lang->id == 2)
            ...
            @foreach($product->galleries as $gal)
                <div class="carousel-item">
                      <img class=" w-100" src="{{asset('assets/images/galleries/'.$gal->photo)}}"  @if(!$slang) @if($lang->id == 2)
                ...
                <div class="col-3">
                    <div class="quick-all-slider">
                        <img class="quick-zoom-gallery" width="80"
                             src="{{filter_var($product->photo, FILTER_VALIDATE_URL) ?$product->photo:asset('assets/images/products/'.$product->photo)}}"  title="The description goes here" data-slide-to="0" data-target="#carouselExampleIndicators">

                    </div>
                </div>


                @foreach($product->galleries as $i=>$gal)
                <div class="col-3">
                    <div class="quick-all-slider">
                        <img class="quick-zoom-gallery" width="80" style="margin-bottom: 5px;"
                             src="{{asset('assets/images/galleries/'.$gal->photo)}}" title="The description goes here"
                             data-slide-to="{{ $i+1 }}" data-target="#carouselExampleIndicators">
                        </div>
                    </div>
                @endforeach
    ...
    <div class="right-area">
      <div class="product-info">
        <h4 class="product-name"><a target="_blank"
            href="{{ route('front.product',['slug' => $product->slug , 'lang' => $sign]) }}">{{ $product->name }}</a>
        </h4>
        ...
        <div class="product-price">
          ...
          <p class="price"><span id="msizeprice">{{ $product->showPrice() }}</span>
            <small><del>{{ $product->showPreviousPrice() }}</del></small>
          </p>
        ...
        @php
        $colors = App\Models\Color::where('product_id',$product->id)->first();

        $sizes = App\Models\Color::where('product_id',$product->id)->where('size_qty','>', 0)->distinct('size')->get();
        ...
        <div class="product-size">
          ...
          <ul class="siz-list siz-lists">
            @php
            $co =
            App\Models\Color::where('product_id',$product->id)->where('size',$product->size[0])->where('size_qty','>',
            0)->get();
            $is_first = true;
            @endphp
        ...
        <div class="product-size">
          ...
          <ul class="siz-list siz-list2">
            @php
            $co = App\Models\Color::where('product_id',$product->id)->where('size',$colors->size)->where('size_qty','>',
            0)->get();
            $is_first = true;
            @endphp
            ...
            <li class="{{ $is_first ? 'active' : '' }} sizes si" id="{{$data1->size}}">
              <span class="box boxz">{{ $data1->size }}
                ...
                <input type="hidden" class="size_key" value="{{$key}}">
                <input type="hidden" class="pro_id" value="{{$product->id}}">
                <input type="hidden" class="size_price" value="{{ round($data1->size_price * $curr->value,2) }}">
              </span>
            </li>
            @endif
            @php
        ...
        <div class="info-meta-3">
          <ul class="meta-list">
            ...
            <li class="d-block count {{ $product->type == 'Physical' ? '' : 'd-none' }}">
              <div class="qty">
                <ul>
                  <li>
                    ...
                    <div class="">
                      ...
                      <div class="custom-control custom-radio">
                        ...
                        <input type="radio" id="{{$attrKey}}{{ $optionKey }}" name="{{ $attrKey }}"
                          class="custom-control-input mproduct-attr" data-key="{{ $attrKey }}"
            ...
            <li class="addtocart">
              <a href="{{ route('affiliate.product',['slug' => $product->slug , 'lang' => $sign]) }}" target="_blank"><i
            ...
            {{-- <li class="addtocart ">
              <a href="{{ route('product.cart.quickadd',['id' => $product->id , 'lang' => $sign ]) }}"><i
                  class="icofont-cart"></i>{{ $langg->lang251 }}</a>
            </li> --}}
            @endif
            ...
            <li class="favorite">
              <a href="javascript:;" class="add-to-wish" data-href="{{ route('user-wishlist-add',$product->id) }}"><i
            ...
            <li class="compare">
              <a href="javascript:;" class="add-to-compare"
                data-href="{{ route('product.compare.add',$product->id) }}"><i class="icofont-exchange"></i></a>
            </li>
          </ul>
